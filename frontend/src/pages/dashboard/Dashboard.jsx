...
const PIPELINE_STAGES = [
  { name: 'Lead', count: 42, color: '#2563eb' },
  { name: 'Qualified', count: 31, color: '#4f46e5' },
  { name: 'Requirement', count: 24, color: '#7e22ce' },
  { name: 'Demo', count: 18, color: '#0891b2' },
  { name: 'Proposal', count: 12, color: '#d97706' },
  { name: 'Negotiation', count: 8, color: '#db2777' },
  { name: 'Won', count: 5, color: '#16a34a' },
];
...
export default function Dashboard() {
  ...
  const kpis = useMemo(() => {
    ...
    const qualified = leads.filter((l) => !['New', 'Assigned', 'Lost'].includes(l.status)).length;
    const activeOpps = opportunities.filter((o) => !['Won', 'Lost'].includes(o.stage)).length;
    const pipelineValue = opportunities.filter((o) => o.stage !== 'Lost').reduce((a, o) => a + o.value, 0);
    const dueFollowUps = followUps.filter((f) => f.status === 'Pending' && f.date === getTodayISO()).length;
    const overdueFollowUps = followUps.filter((f) => f.status === 'Pending' && f.date < getTodayISO()).length;
    return { totalLeads, qualified, activeOpps, pipelineValue, dueFollowUps, overdueFollowUps };
  }, [leads, opportunities, followUps]);

  const TODAY = getTodayISO();
  const todayList = followUps.filter((f) => f.status === 'Pending' && f.date === TODAY).slice(0, 5);
  const overdueList = followUps.filter((f) => f.status === 'Pending' && f.date < TODAY).slice(0, 4);
    ...
    <div className="page">
      ...
      <div className="grid mb-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>
        {loading ? (
          ...
          <>
            ...
            <StatCard icon="bi-shield-check" iconColor="#7e22ce" iconBg="#7e22ce15" value={kpis.qualified} label="Qualified Leads" onClick={() => navigate('/leads')} />
            ...
            <StatCard icon="bi-wallet2" iconColor="#16a34a" iconBg="#16a34a15" value={formatINR(kpis.pipelineValue)} label="Pipeline Value" onClick={() => navigate('/opportunities')} />
      ...
      <div className="grid grid-2 mb-4">
        ...
        <div className="surface p-4">
          <div className="d-flex align-items-center justify-content-between mb-3">
            <div>
              <div className="fw-7 fs-18">Today's Follow-ups</div>
              <div className="fs-12 text-muted-c">Your immediate action items</div>
            </div>
            <button className="btn btn-light btn-sm" onClick={() => navigate('/activities')}>All</button>
          </div>
          {loading ? <Skeleton h={220} /> : (
              ...
              {overdueList.map((f) => (
                <div key={f.id} className="d-flex align-items-center gap-3 py-2 px-3 bg-white" style={{ cursor: 'pointer' }} onClick={() => navigate('/activities/followups')}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="fw-6 text-truncate fs-14">{f.relatedEntityName}</div>
                    <div className="fs-12 text-muted-c text-truncate">{f.action}</div>
                  </div>
                  <div className="text-end">
                    <div className="fs-12 fw-6 text-danger">{formatDate(f.date, { short: true })}</div>
                  </div>
                </div>
              ))}
              {todayList.map((f) => (
                <div key={f.id} className="d-flex align-items-center gap-3 py-2 px-3 bg-white" style={{ cursor: 'pointer' }} onClick={() => navigate('/activities/followups')}>
                  ...
                  <div className="text-end">
                    <div className="fs-12 fw-6">{f.time || 'Anytime'}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="d-flex align-items-center flex-wrap h-100 pb-4">
              <div style={{ width: 220, height: 220 }}>
                <PieChart width={220} height={220}>
                  <Pie data={SOURCE_DATA} dataKey="value" nameKey="name" innerRadius={60} outerRadius={90} paddingAngle={2} stroke="none">
                    {SOURCE_DATA.map((e) => <Cell key={e.name} fill={e.color} />)}
                  </Pie>
